...
function getCellTextAreaElement(cellValue, cellAddress) {
  ...
  textAreaElement.ondragstart = function (ev){
...
function getColumnNameInputElement(columnName, cellAddress) {
  ...
  columnNameInputElement.ondragstart = function (ev) {
    ev.dataTransfer.setData("text/plain", ev.target.id);
  };
  ...
  tableState.body.forEach((element) => {
    ...
    element.columnDetails.forEach((cellDetail) => {
      ...
      tdElement.ondrop = function (ev) {
        const [rowNumber, columnNumber] = ev.dataTransfer.getData("text/plain").split("-");
        const [dropZoneRowIdx, dropZoneColumnIdx] = ev.target.id.split("-");
        let swapValue = tableState.body[rowNumber].columnDetails[columnNumber].cellValue
        tableState.body[rowNumber].columnDetails[columnNumber].cellValue = tableState.body[dropZoneRowIdx].columnDetails[dropZoneColumnIdx].cellValue
        tableState.body[dropZoneRowIdx].columnDetails[dropZoneColumnIdx].cellValue = swapValue;
        updateTableDataInLocalStorage(tableState);
      };
